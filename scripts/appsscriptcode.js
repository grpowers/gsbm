...
function manualRefresh() {
  var sheet = SpreadsheetApp.getActive().getSheetByName("_LIVE");
  var cell = sheet.getRange("Z1"); // Moved from A1 to Z1
  var val = cell.getValue();
  cell.setValue(val === 1 ? 0 : 1); 
}

function fastSortLogs() {
  ...
  if (lastRow <= 1) {
    SpreadsheetApp.getUi().alert("No data found to sort below the header!");
    return;
  }
  ...
  try {
    if (sheetName === "Straights Picks Log") {
      // Sort Straights: Date (1) > Sport (2) > Game (3)
      range.sort([
        {column: 1, ascending: true}, 
        {column: 2, ascending: true}, 
        {column: 3, ascending: true}
      ]);
      ss.toast("Sorted: Date > Sport > Game", "Straights Log Success");
    } else {
      // Sort Sport Logs: Date (1) > Game (3) > Source (4)
      range.sort([
        {column: 1, ascending: true}, 
        {column: 3, ascending: true}, 
        {column: 4, ascending: true}
      ]);
      ss.toast("Sorted: Date > Game > Source", "Sport Log Success");
    }
  } catch (e) {
    SpreadsheetApp.getUi().alert("Sort Error: " + e.message);
  }
}
...
function onOpen() {
  ...
  // Your existing System Tools menu
  ui.createMenu('🚀 System Tools')
      .addItem('Advanced Sort Logs', 'fastSortLogs')
      .addToUi();
  
  // Your new Sports Tools menu
  ui.createMenu('🏆 Sports Tools')
      .addItem('Update Game Outcomes', 'updateAllOutcomes')
      .addToUi();

// Consolidating everything into one "Management" menu
  ui.createMenu('🛡️ System Guard')
      .addItem('Sync & Validate Logs', 'validatePicksData')
      .addToUi();
}